...
export class WalletProvider {
        ...
        if (walletData?.success && walletData?.data) {
          ...
          const portfolio = {
            ...
            items: items.sort((a, b) =>
              BigNumber(b.valueUsd).minus(BigNumber(a.valueUsd)).toNumber(),
            ),
          };
  ...
  async fetchPortfolioValueCodex(runtime): Promise<WalletPortfolio> {
    try {
      ...
      // Fetch token prices
      const prices = await this.fetchPrices(runtime);
      const solPriceInUSD = BigNumber(prices.solana.usd.toString());
      ...
      const totalUsd = items.reduce(
        (sum, item) => sum.plus(BigNumber(item.valueUsd)),
        BigNumber(0),
      );
  ...
  formatPortfolio(runtime, portfolio: WalletPortfolio, prices: Prices): string {
    ...
    const totalUsdFormatted = BigNumber(portfolio.totalUsd).toFixed(2);
    ...
    const nonZeroItems = portfolio.items.filter((item) =>
      BigNumber(item.uiAmount).isGreaterThan(0),
    );
    ...
    if (nonZeroItems.length === 0) {
      ...
      for (const item of nonZeroItems) {
        const valueUsd = BigNumber(item.valueUsd).toFixed(2);
        output += `${item.name} (${item.symbol}): ${BigNumber(
          item.uiAmount,
    ...
    output += '\nMarket Prices:\n';
    output += `SOL: $${BigNumber(prices.solana.usd).toFixed(2)}\n`;
    output += `BTC: $${BigNumber(prices.bitcoin.usd).toFixed(2)}\n`;
    output += `ETH: $${BigNumber(prices.ethereum.usd).toFixed(2)}\n`;
