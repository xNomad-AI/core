...
import { WalletProvider, type Item } from './wallet.js';
...
export class TokenProvider {
  ...
  async fetchTokenCodex(): Promise<TokenCodex> {
    try {
      ...
      elizaLogger.error('Error fetching token data from Codex:', error.message);
  ...
  async getProcessedTokenData(): Promise<ProcessedTokenData> {
    try {
      elizaLogger.log(`Fetching security data for token: ${this.tokenAddress}`);
      const security = await this.fetchTokenSecurity();

      const tokenCodex = await this.fetchTokenCodex();

      elizaLogger.log(`Fetching trade data for token: ${this.tokenAddress}`);
      ...
      elizaLogger.log(`Checking recent trades for token: ${this.tokenAddress}`);
