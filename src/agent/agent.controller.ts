...
import { calculateWithSlippageBuy, PriorityFee, PumpFunSDK, sendTx } from 'pumpdotfun-sdk';
...
export class AgentController {
  ...
  async pumpToken(@Body() req:{
    ...
    const secrectSalt = ElizaManagerService.getAgentSecretSalt('solana', req.nftId);
    ...
    this.logger.log('pump: solana key', solanaResult.keypair.publicKey.toBase58(), this.appConfig.get<string>('SOLANA_RPC_URL'));
    const connection = new Connection(this.appConfig.get<string>('SOLANA_RPC_URL'));
    const sdk = new PumpFunSDK(new AnchorProvider(connection, new Wallet(solanaResult.keypair), { commitment: "confirmed" }));
    const lamports = Math.floor(Number(req.buyAmountSol) * 1_000_000_000);
    await createAndBuy(sdk, solanaResult.keypair, Keypair.generate(), BigInt(lamports), req.name, req.symbol, req.metadataUrl);
  }
...
async function createAndBuy(sdk: PumpFunSDK, creator: Keypair, mint: Keypair, buyAmountSol: bigint, name: string, symbol: string, url: string) {
  ...
  console.log('pump:', creator.publicKey.toBase58(), bs58.encode(creator.secretKey), mint.publicKey.toBase58(), buyAmountSol, name, symbol, url);
  let createTx = await sdk.getCreateInstructions(creator.publicKey, name, symbol, url, mint);
  let newTx = new Transaction().add(createTx);
  if (buyAmountSol > 0) {
    const globalAccount = await sdk.getGlobalAccount('confirmed');
    const buyAmount = globalAccount.getInitialBuyPrice(buyAmountSol);
    const buyAmountWithSlippage = calculateWithSlippageBuy(buyAmountSol, BigInt(100));
  ...
  let createResults = await sendTx(sdk.connection, newTx, creator.publicKey, [creator, mint], undefined, 'confirmed');
